...
use App\Importers\ShopImporter;
...
class ShopImporterTest extends TestCase
{
    ...
    public function testImportWithUpdateOfferLogErrorMessage(): void
    {
        ...
        Log::shouldReceive('error')->once()->with('Unable to update Shop.', ['external_id' => 789]);
        ...
        $this->mock(ShopService::class, function (MockInterface $mock) use ($shop, $dto) {
            $mock->shouldReceive('findByExternalId')->once()->with(789)->andReturn($shop);
            $mock->shouldReceive('update')->once()->with($shop, $dto->toArray())->andReturn(false);
        });
