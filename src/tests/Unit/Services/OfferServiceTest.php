...
class OfferServiceTest extends TestCase
{
    ...
    {
        $this->mock(OfferRepository::class, function (MockInterface $mock) {
            $mock->shouldReceive('getOffersByShopCountry')->once()->with('UK')->andReturn($this->createStub(Collection::class));
        });

        /** @var OfferService $service */
        $service = $this->app->make(OfferService::class);

        $service->getOffersByShopCountry('UK');
    }

    public function testGetOffersByShop(): void
    {
        /** @var Shop $shop */
        $shop = Shop::factory()->create(['id' => 'foo']);

        $this->mock(OfferRepository::class, function (MockInterface $mock) {
            $mock->shouldReceive('getOffersByShopId')->once()->with('foo')->andReturn($this->createStub(Collection::class));
        });

        /** @var OfferService $service */
        $service = $this->app->make(OfferService::class);

        $service->getOffersByShop($shop);
    }
}
